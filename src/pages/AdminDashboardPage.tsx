          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {users.map(user => (
              <tr key={user.id} className={selected?.id === user.id ? 'bg-emerald-50' : ''}>
                <td className="px-6 py-4 whitespace-nowrap font-medium">{(user as any).firstName || user.name || '-'}</td>
                <td className="px-6 py-4 whitespace-nowrap font-medium">{(user as any).lastName || '-'}</td>
